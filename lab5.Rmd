...
paste(ipps$Total.Discharges[1], "refers to a number of discharges in a hospital in my dataset.")
...
```{r eval=FALSE}
Filter on the diagnosis group (DRG.Definition) and filter it to the hospital names (Provider.Name)
```
...
ipps %>% filter(DRG.Definition == "025 - CRANIOTOMY & ENDOVASCULAR INTRACRANIAL PROCEDURES W MCC") %>% select(Total.Discharges) %>% summary()

#If you have qualified units of observation
ipps %>% filter(DRG.Definition == "025 - CRANIOTOMY & ENDOVASCULAR INTRACRANIAL PROCEDURES W MCC" & Provider.Name == "SOUTHEAST ALABAMA MEDICAL CENTER") %>% select(Total.Discharges) %>% summary()
...
```{r eval=FALSE}
How prevalant a certain diagnosis is assigned in this case craniotomy and enodvascular procedures. 
```
...
```{r eval=FALSE}
May need to take into consideration that DRGs are seperated by seperated also by with or without major complications. So W/O CC or W CC & MCC. If I wanted to summarize a certain kind of diagnosis I would have to consider how they're divided.
```
...
```{r eval=FALSE}
Need to know what qualifies as discharge and the process of how physcians formally assign a specific DRG to a patients case. 
```

What insight can you draw from calling summary() on your filtered dataset?

```{r eval=FALSE}
Which DRGs are the most prevalent in a city or state.
```
...
ipps %>% filter(Total.Discharges >= 600) %>% distinct(DRG.Definition)
...
```{r eval=FALSE}
Which diagnosis are most frequently addressed by healthcare facilities.
```
...
ipps %>% group_by(Provider.State) %>% summarize(State_Discharges = sum(Total.Discharges, na.rm = TRUE), OBSERVATIONS = n(), MISSING = sum(is.na(Total.Discharges)), PERCENT_MISSING = sum(is.na(Total.Discharges))/n()*100)
...
```{r eval=FALSE}
Which states have the most or least amount of discharges. 
```
...
ipps %>% 
  filter(Average.Medicare.Payments > 90000.00) %>%
  group_by(DRG.Definition) %>% summarize(State_Discharges = sum(Total.Discharges, na.rm = TRUE), OBSERVATIONS = n(), MISSING = sum(is.na(Total.Discharges)), PERCENT_MISSING = sum(is.na(Total.Discharges))/n()*100) %>% select(DRG.Definition)
```

What question might this analysis help to address?

```{r eval=FALSE}
Which diagnosis is costing the most for medicare compared to discharges.
```
...
ipps %>% ggplot(aes(x = Provider.State)) + geom_bar() +
labs(title = "Number of DRGs per State", x = "States", y = "Count of DRGs") + #
  theme_bw() +
...
```{r eval=FALSE}
States with larger populations are more represented in this graph. This could be due to them having to handle more cases with patients. Thus, they would have more variety in DRGs.
```
...
```{r eval=FALSE}
Some cases that are very similar cause are categorized in many different ways. Such as drug use with could be placed as rehabilitation, psychoses,  heartfailure and shock, etc. This affects the data greatly because the primary diagnosis is decided based upon the condition that doctors found to be the primary focus of a patient's stay.   
```
...
ipps %>% ggplot(aes(x = Total.Discharges)) + geom_freqpoly(binwidth = 400) + facet_wrap(~DRG.Definition)
```
...
```{r eval=FALSE}
The distribution of total discharges for each DRG. 
```
...
```{r eval=FALSE}
I selected this bindwith because it had the most readability. Not much changes when the bindwith is small. 
```
...
```{r eval=FALSE}
"The number of discharges billed by the provider for inpatient hospital services." If discharges were defined as an average instead of a total the distrbution of the variable would be lost and appear differently. 
```
...
hospitals %>% 
  filter(STATUS == "OPEN") %>%
  ggplot(aes(x = TYPE, y = BEDS)) + 
  geom_jitter(size = 0.5, alpha = 0.7) + #Change geom_point to geom_jitter, reduce the size, add transparency for overplotting
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust=1)) +
  labs(title = "Number of Beds in Hospitals by Type", x = "Type", y = "Number of Beds")
...
ipps %>% 
  ...
  ggplot(aes(x = Provider.City, y = Average.Total.Payments, col = as.factor(Provider.Name))) + 
  geom_jitter(size = 0.5, alpha = 0.5) + #Change geom_point to geom_jitter, reduce the size, add transparency for overplotting
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust=1)) +
  labs(title = "Measure of Average Total Payments in DRG by Cities in Massachusetts", x = "", y = "") +
...
```{r eval=FALSE}
Which cities in the state are getting the most payments for their patients.  
```

Are there any other variables in your dataset that you need to take into consideration before directing this analysis towards answering that question? In other words, do you need to zoom into any specific areas of the dataset (by filtering) in order to appropriately address this question? If so, which? Be sure to adjust your plot above to reflect this.

```{r eval=FALSE}
Yes, I would need to consider other variables like Average.Covered.Charges and Average.Medicare.Payments and see if they correlate.  
```

What else would we need to know to fully address this question? Here you may consider what you know about how this dataset was produced and its limitations.

```{r eval=FALSE}
I would need to know what the outliers in the data are and why they exist. Addtionally, maybe get some info on how physicians in state assign a DRG to a patient. 
```

What insight can you draw from this plot?

```{r eval=FALSE}
City of Boston predictably gets the most payments followed by Worcester and Springfield.  
```

```{r fig.height=8, fig.width=10}
#Fill the code for plot 2 here. Add a title and labels to your plots. Be sure to adjust for overplotting.
ipps %>% 
  filter(DRG.Definition %in% c("037 - EXTRACRANIAL PROCEDURES W MCC","103 - HEADACHES W/O MCC", "180 - RESPIRATORY NEOPLASMS W MCC", "069 - TRANSIENT ISCHEMIA")) %>%
  ggplot(aes(x = Average.Medicare.Payments, y = Average.Total.Payments, col= DRG.Definition)) + 
  geom_point(size = 0.5, alpha = 0.5) +
  theme_bw() +
  labs(title = "Relationship between Hospital DRGs Total Discharges and Average Total Payments", x = " Total Discharges", y = "___") 
```

What question might this analysis help to address?

```{r eval=FALSE}
Is there a relationship between total discharges and payments for a Hospital DRG?
```

Are there any other variables in your dataset that you need to take into consideration before directing this analysis towards answering that question? In other words, do you need to zoom into any specific areas of the dataset (by filtering) in order to appropriately address this question? If so, which? Be sure to adjust your plot above to reflect this.

```{r eval=FALSE}
Maybe need to look at Averge.Medicare.Payments but will most likeley yield similar results of no correlation.  
```

What else would we need to know to fully address this question? Here you may consider what you know about how this dataset was produced and its limitations.

```{r eval=FALSE}
It would be helpful to know how discharges are counted exactly. If a patient can be discharged more than once during their stay.  
```

What insight can you draw from this plot?

```{r eval=FALSE}
No correlation between discharges and average total payments
```
...
ui <- dashboardPage(
  ...
  dashboardHeader(title = "Healthcare Data"),
...
server <- function(input, output, session) {
  ...
  ipps %>% 
  filter(Provider.State == "MA") %>%
  ggplot(aes(x = Provider.City, y = Average.Total.Payments)) + 
  geom_jitter(size = 0.5, alpha = 0.7) + #Change geom_point to geom_jitter, reduce the size, add transparency for overplotting
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust=1)) +
  labs(title = "Measure of Average Total Payments in DRG by Cities in Massachusetts", x = "", y = "") 
  ...
  geom_point(size = 0.5) +
  theme_bw() +
  labs(title = "Relationship between Hospital DRGs Total Discharges and Average Total Payments", x = " Total Discharges", y = "Average Total Payments") 
  #REPLACE ABOVE WITH YOUR OWN PLOT 
  })
