...
For these projects, all data will be accessed via hosted services so that we need not worry about working with different file paths and working directories on our computers. 

For our example datsets:

* The Johns Hopkins team has made the confirmed Covid-19 cases dataset available in their own GitHub repo.
...
I can import all of these datasets into RStudio by reading these files in as a CSV, using the function **read.csv("[PATH_TO_DATE_FILE]")**. When we read this data file into R, it will import as a data frame (reference lab 1 if you do not know what a data frame is). We will store that data frame in a variable using **<-** so that we may reference the dataset throughout the lab.
...
I've included the code to import the data for each of your projects. Uncomment the line associated with your data frame below by removing the '#' from the front of the code associated with your topic. Then import the data for your topic by running the code chunk. **I encourage you to only run the code for your topic so as not to over-burden RStudio Cloud.**

### Domestic Violence

```{r}
#If this is your group, uncomment the line below and run the code chunk.
...
```{r}
#Uncomment the line associated with your topic's data frame below by removing the '#' in front of the line. Then run the code chunk to check out the structure. 

#str(df)

#str(dom_violence_calls)
#str(internet_master_plan)
str(ipps)
#str(snap)
#str(self_sufficiency_ca)
#str(aqi)
```
...
For most of your datasets, everything imported as the correct type. However, for the Income Security team, we are seeing several numeric variables that imported as chrs. For those in the Income Security group, can you figure out why?

This often happens when there are characters like commas, dollar signs, percent signs in the column. In the self-sufficiency dataset, we have several such characters importing with the number. We need to strip these characters before converting the variable to numeric. 

Let's remove the characters that are appearing in these column with the gsub function, which replaces a character with another character - in this case with nothing. Only the Income Security group need follow the instructions in the code chunk below.

```{r}
#Income security group only: uncomment the last line below, and run the code chunk.
...
Let's go ahead and convert that numeric ZIP variable into a char by casting the variable **as.character()**.
...
Following the same pattern, change the type of any incorrectly typed variables in your own dataset. Only the income security group need follow the instructions below.

```{r}
#Income security group only: Uncomment the last line, and run the code chunk.
...
None of you need to do this in your own datasets. 
...
```{r}
#Run to check out the first ten rows of cases.

cases %>% head(10)
```

To do this, we will select all of the values equal to ("==") "NOT AVAILABLE" in the hospitals dataset, equal to ("==") "" (empty string) in the cases dataset, or equal to ("==") -999 in the hospitals dataset, and convert them to NA. Check out how I do this below. 
...
The only two groups that have incorrectly formated NA values in their datasets are the Infrastructure group and the Food Security group. Let's convert those to greyed out NAs.

```{r}
#Infrastructure group and food security group only: Uncomment the line associate with your data frame, and run the code chunk.
...
Check out the format of your date. Is it just a year? Just a month? A year, month, and day? Are there times listed? What order are each of these values listed in? [This link](https://lubridate.tidyverse.org/) offers more information about how to structure date conversions. If the date is just a year, we can leave it as an int. Otherwise, we will convert the date to a date format. I've gone ahead and converted dates into the proper format for the three groups with such dates below: Environment, Food Security and Domestic abuse. If you are in one of these three groups follow, the instructions below:

```{r}
#Uncomment the line associated with your dataframe, and run the code chunk.
...
hospitals %>% select(ID, NAME, COUNTY, NAICS_CODE, TYPE) %>% head(10)

ipps %>% select(DRG.Definition, Provider.Id, Provider.Street.Address) %>% head(10)
...
ipps %>% select(Total.Discharges, Average.Covered.Charges, Average.Total.Payments) %>% head(10)
...
#ca_arrests %>%  filter(YEAR == 2001 & COUNTY == "Sacramento County" & GENDER == "Male" & RACE == "White" & ARREST_DISP_CODE == "Released" & AGE_GROUP == "20 to 29")
```
...
paste("I have", nrow(ipps), "unique hospital-specific charges for DRGs represented in my dataset.")
```
...
n_unique_keys <- ipps %>% select(DRG.Definition, Provider.Id) %>% n_distinct()
...
paste("I have", nrow(ipps), "unique DRGs and Provider ids represented in my dataset.")
```
...
How are the observational units in your dataset defined? Note that if you have multiple variable constituting your observational unit, you may select just one to compose your response. For instance, how are agency names defined in the domestic violence data? How are dates or counties defined in the AQI data? How are neighborhoods defined in the infrastructure data? How are DRGs defined in the healthcare data? How are states defined in the SNAP data? How are family types defined in the income security data? Be sure to refer to the data documentation. 

```{r eval=FALSE}
The Diagnosis related group (DRG) is a payment category that CMS uses for the purpose reimbursing hospitals. The DRG manual lists hundreds of different codes and also makes a distinction between hospital care with a complication or comorbidity (CC) and those with a major complication (MCC). 
```

Who or what organization manages these definitions? In other words, who gets to decide what counts in this data? 

```{r eval=FALSE}
The Center for Medicare and Medicaid Services (CMS) facilitates the organization of DRGs. 
```
...
```{r eval=FALSE}
The DRG system was developed to help get the appropiate amount of payments to hospitals for their care. Many factors are considered in deciding what DRG code to assign a patient. It is the responsibility of the hospital and its phycians to accurately report the level of a patients condition, whether there were complications, if a patient is low income, etc. The DRG system helps CMS quantify hospital care and control physicians manners. source:(https://vincari.com/media/diagnosis-related-groups-care/)
```
...
#ipps dataset has no columns with NAs
